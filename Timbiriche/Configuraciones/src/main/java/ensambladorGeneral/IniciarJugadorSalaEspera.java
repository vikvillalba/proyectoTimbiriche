...
/**
 * Main para iniciar el juego como JUGADOR (no host) directamente en sala de espera. Utiliza config_partida1.properties para obtener la configuración de red.
 *
 * Diferencias con IniciarHost: - El jugador NO es host (esHost = false) - Se suscribe a eventos de jugador (EN_SALA_ESPERA, SOLICITAR_UNIRSE, RESULTADO_CONSENSO) - Carga configuración desde archivo properties en lugar de valores hardcodeados
 *
 * Diferencias con IniciarCliente: - Abre directamente FrmSalaEspera en lugar de FrmMenuInicio - Tiene un jugador predefinido (no requiere proceso de búsqueda de host)
 *
