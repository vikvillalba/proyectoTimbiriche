...
public class FrmPartida extends JFrame implements ObservadorInicioPartida, ObservadorTurno {
    ...
    public FrmPartida(IModeloJugadoresLectura modeloJugadores, IModeloTableroLectura modeloTablero, ControladorPartida controlador) {
        ...
        setTitle("Partida en curso - Sesión de " + sesion.getNombre());
        // Deshabilitar tablero si no es el turno del jugador sesion
        actualizarEstadoTablero();

        actualizarJugadoresIniciales(modeloJugadores);
    }

    private void actualizarJugadoresIniciales(IModeloJugadoresLectura modeloJugadores) {

        List<JugadorPresentable> jugadoresIniciales = modeloJugadores.getJugadores();
        if (jugadoresIniciales != null && !jugadoresIniciales.isEmpty()) {
            jugadores.actualizar(jugadoresIniciales);
            for (JugadorPresentable jugadoresIniciale : jugadoresIniciales) {
                System.out.println(jugadoresIniciale.toString());
            }
        }
    }
    ...
    private void actualizarEstadoTablero() {
        ...
        if (estaEnTurno) {
            System.out.println("[" + sesion.getNombre() + "] Es tu turno bro");
        } else {
